...
const workspaceSource = path.join(rootDir, 'problems', 'form-capture', 'workspace');
const gradingDir = path.join(rootDir, 'grading', 'form-capture');
...
async function ensureDependencies(dir: string): Promise<void> {
  ...
  const result = await runCommand('npm install', 'npm', ['install'], { cwd: dir, env });
...
async function copyWorkspace(src: string): Promise<string> {
  const tmpRoot = await mkdtemp(path.join(os.tmpdir(), 'llxprt-form-'));
...
async function main(): Promise<void> {
  ...
  const runResultsDir = path.join(resultsDir, `form-capture-${runId}`);
  ...
  for (const profile of selectedProfiles) {
    ...
    if (installResult.exitCode !== 0) {
      ...
      results.push({
        ...
        status: 'fail',
        workspaceArchive: path.join(`results/form-capture-${runId}`, profile.name, 'workspace')
      });
      continue;
    }

...
const workspaceSource = path.join(rootDir, 'problems', 'pagination', 'workspace');
const gradingDir = path.join(rootDir, 'grading', 'pagination');
...
async function copyWorkspace(src: string): Promise<string> {
  const tmpRoot = await mkdtemp(path.join(os.tmpdir(), 'llxprt-pagination-'));
...
async function main(): Promise<void> {
  ...
  const runResultsDir = path.join(resultsDir, `pagination-${runId}`);
  ...
  for (const profile of PROFILES) {
    ...
    for (const cmd of gradeCommands) {
